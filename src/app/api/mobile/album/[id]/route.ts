...
import { favoritesFor } from "@/lib/collections";
...
export const GET = async (
  ...
  const [community, reviews, favorites] = await Promise.all([
    communityRating(id),
    reviewsForAlbum(id, userId ?? null),
    userId ? favoritesFor(userId, id) : Promise.resolve([]),
  ]);

  return NextResponse.json({
    ...album, existing, popularity, community, reviews, favorites,
  });
};
